...
int zjb_submit(ZJB *zjb, string contents, string &jobId)
{
  ...
  // cout << "@TEST " << string(cjobid) << endl;

  // char cjobcorr[64 + 1] = {0};
  // rc = ZJBSYMB(zjb, "SYS_CORR_LASTJOB", cjobcorr);
  // cout << "@TEST " << string(cjobcorr) << endl;
...
void zjb_build_job_response(ZJB_JOB_INFO *PTR64 job_info, int entries, vector<ZJob> &jobs)
{
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    strncpy(tempJobCorr, (char *)job_info_next[i].statjqtr.sttrjcor, sizeof(job_info->statjqtr.sttrjcor));
    ...
    string job_correlator(tempJobCorr);
