...
static int copy_sequential(ZDS *zds, const std::string &src_dsn, const std::string &dst_dsn)
{
  ...
  if (!fin)
  {
    ZDIAG_SET_MSG(&zds->diag, "Could not open source '%s'", src_dsn.c_str());
  ...
  {
    fclose(fin);
    ZDIAG_SET_MSG(&zds->diag, "Could not open target '%s'", dst_dsn.c_str());
    return RTNCD_FAILURE;
  }
  ...
  while ((bytes_read = fread(buffer, 1, sizeof(buffer), fin)) > 0)
  {
    ...
    if (bytes_written != bytes_read)
    {
      fclose(fin);
      fclose(fout);
      ZDIAG_SET_MSG(&zds->diag, "Write error copying to '%s'", dst_dsn.c_str());
      return RTNCD_FAILURE;
    }
  }
...
static int delete_all_members(ZDS *zds, const std::string &pds_dsn)
{
  ...
  for (size_t i = 0; i < members.size(); i++)
  {
    ...
    if (rc != RTNCD_SUCCESS)
    {
      ZDIAG_SET_MSG(&zds->diag, "Failed to delete member '%s'", member_dsn.c_str());
      return RTNCD_FAILURE;
    }
  }
  return RTNCD_SUCCESS;
...
int zds_write_to_dd(ZDS *zds, std::string ddname, const std::string &data)
{
  ...
  if (!out.is_open())
  {
    ZDIAG_SET_MSG(&zds->diag, "Could not open file '%s'", ddname.c_str());
    return RTNCD_FAILURE;
  }
...

  {
    ...
    if (!data.empty())
    {
      ...
      if (has_encoding)
      {
        ...
        {
          ZDIAG_SET_MSG(&zds->diag, "Failed to convert input data from %s to %s", source_encoding.c_str(), codepage.c_str());
          return RTNCD_FAILURE;
        }
      }
      ...
      {
        ZDIAG_SET_MSG(&zds->diag, "Failed to write to '%s' (possibly out of space)", dsname.c_str());
        return RTNCD_FAILURE;
      }
    }
  }
...
static int zds_write_member_bpam(ZDS *zds, const std::string &dsn, std::string &data)
{
  ...
  {
    ZDIAG_SET_MSG(&zds->diag, "Cannot open converter from %s to %s", source_encoding.c_str(), codepage.c_str());
...
int zds_write_to_dsn(ZDS *zds, const std::string &dsn, std::string &data)
{
  ...
  {
    ZDIAG_SET_MSG(&zds->diag, "Could not access '%s'", dsn.c_str());
    return RTNCD_FAILURE;
  }
  ...
  {
    ZDIAG_SET_MSG(&zds->diag, "Writing to RECFM=%s data sets is not supported", attrs.recfm.c_str());
