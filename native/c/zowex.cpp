...
int handle_tso_issue(ZCLIResult result)
{
  ...
  vector<string> dds;
  dds.push_back("alloc fi(systsprt) path('" + systsprt + "') pathopts(ordwr,ocreat,otrunc,osync) pathmode(sirusr,siwusr,sirgrp) filedata(text) msg(2)");
  // dds.push_back("alloc fi(systsprt) da('dkelosky.zowex') DSORG(PS) SPACE(5,5) CYL LRECL(80) RECFM(F,B) new keep");
