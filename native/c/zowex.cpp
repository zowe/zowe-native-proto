...
int run_interactive_mode(ArgumentParser &arg_parser,
...
int main(int argc, char *argv[]) {
  ...
  arg_parser.get_root_command().add_keyword_arg(
      "interactive", make_aliases("--interactive", "--it"),
      "interactive (REPL) mode", ArgType_Flag, false, ArgValue(false));

  // Console command group
  auto console_cmd =
      command_ptr(new Command("console", "z/OS console operations"));
  ...
  issue_cmd->add_keyword_arg("wait", make_aliases("--wait"),
  ...
  tso_issue_cmd->add_positional_arg("command", "command to issue",
                                    ArgType_Single, true);
  ...
  auto data_set_cmd =
      command_ptr(new Command("data-set", "z/OS data set operations"));
  ...
  auto response_format_csv_option =
      make_aliases("--response-format-csv", "--rfc");
  auto response_format_bytes_option =
  ...
  ds_create_cmd->add_positional_arg(
      "dsn", "data set name, optionally with member specified", ArgType_Single,
      true);

  // Data set creation attributes
  ds_create_cmd->add_keyword_arg("alcunit", make_aliases("--alcunit"),
                                 "Allocation unit", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("blksize", make_aliases("--blksize"),
                                 "Block size", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("dirblk", make_aliases("--dirblk"),
                                 "Directory blocks", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("dsorg", make_aliases("--dsorg"),
                                 "Data set organization", ArgType_Single,
                                 false);
  ds_create_cmd->add_keyword_arg("primary", make_aliases("--primary"),
                                 "Primary space", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("recfm", make_aliases("--recfm"),
                                 "Record format", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("lrecl", make_aliases("--lrecl"),
                                 "Record length", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("dataclass", make_aliases("--dataclass"),
                                 "Data class", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("unit", make_aliases("--unit"), "Device type",
                                 ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("dsntype", make_aliases("--dsntype"),
                                 "Data set type", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("mgntclass", make_aliases("--mgntclass"),
                                 "Management class", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("dsname", make_aliases("--dsname"),
                                 "Data set name", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("avgblk", make_aliases("--avgblk"),
                                 "Average block length", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("secondary", make_aliases("--secondary"),
                                 "Secondary space", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("size", make_aliases("--size"), "Size",
                                 ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("storclass", make_aliases("--storclass"),
                                 "Storage class", ArgType_Single, false);
  ds_create_cmd->add_keyword_arg("vol", make_aliases("--vol"), "Volume serial",
                                 ArgType_Single, false);
  ds_create_cmd->set_handler(handle_data_set_create);
  data_set_cmd->add_command(ds_create_cmd);

  // Create-vb subcommand
  auto ds_create_vb_cmd = command_ptr(new Command(
      "create-vb",
      "create VB data set using defaults: DSORG=PO, RECFM=VB, LRECL=255"));
  ds_create_vb_cmd->add_alias("cre-vb");
  ds_create_vb_cmd->add_positional_arg(
      "dsn", "data set name, optionally with member specified", ArgType_Single,
      true);
  ...
  auto ds_create_adata_cmd = command_ptr(new Command(
      "create-adata",
      "create VB data set using defaults: DSORG=PO, RECFM=VB, LRECL=32756"));
  ds_create_adata_cmd->add_alias("cre-a");
  ds_create_adata_cmd->add_positional_arg(
      "dsn", "data set name, optionally with member specified", ArgType_Single,
      true);
  ...
  auto ds_create_loadlib_cmd = command_ptr(new Command(
      "create-loadlib",
      "create loadlib data set using defaults: DSORG=PO, RECFM=U, LRECL=0"));
  ds_create_loadlib_cmd->add_alias("cre-u");
  ds_create_loadlib_cmd->add_positional_arg(
      "dsn", "data set name, optionally with member specified", ArgType_Single,
      true);
  ...
  auto ds_create_member_cmd =
      command_ptr(new Command("create-member", "create member in data set"));
  ds_create_member_cmd->add_alias("cre-m");
  ds_create_member_cmd->add_positional_arg(
      "dsn", "data set name with member specified", ArgType_Single, true);
  ...
  ds_view_cmd->add_positional_arg(
      "dsn", "data set name, optionally with member specified", ArgType_Single,
      true);
  ds_view_cmd->add_keyword_arg("encoding", encoding_option,
                               "return contents in given encoding",
                               ArgType_Single, false);
  ds_view_cmd->add_keyword_arg("response-format-bytes",
                               response_format_bytes_option,
                               "returns the response as raw bytes",
                               ArgType_Flag, false, ArgValue(false));
  ds_view_cmd->add_keyword_arg(
      "return-etag", return_etag_option,
      "Display the e-tag for a read response in addition to data", ArgType_Flag,
      false, ArgValue(false));
  ds_view_cmd->add_keyword_arg(
      "pipe-path", pipe_path_option,
      "Specify a FIFO pipe path for transferring binary data", ArgType_Single,
      false);
  ...
  ds_list_cmd->add_positional_arg("dsn", "data set name pattern",
                                  ArgType_Single, true);
  ds_list_cmd->add_keyword_arg("attributes", make_aliases("--attributes", "-a"),
                               "display data set attributes", ArgType_Flag,
                               false, ArgValue(false));
  ds_list_cmd->add_keyword_arg(
      "max-entries", make_aliases("--max-entries", "--me"),
      "max number of results to return before warning generated",
      ArgType_Single, false);
  ds_list_cmd->add_keyword_arg("warn", make_aliases("--warn"),
                               "warn if truncated or not found", ArgType_Flag,
                               false, ArgValue(true));
  ds_list_cmd->add_keyword_arg("response-format-csv",
                               response_format_csv_option,
  ...
  auto ds_list_members_cmd =
      command_ptr(new Command("list-members", "list data set members"));
  ds_list_members_cmd->add_alias("lm");
  ds_list_members_cmd->add_positional_arg("dsn", "data set name",
                                          ArgType_Single, true);
  ds_list_members_cmd->add_keyword_arg(
      "max-entries", make_aliases("--max-entries", "--me"),
      "max number of results to return before warning generated",
      ArgType_Single, false);
  ds_list_members_cmd->add_keyword_arg("warn", make_aliases("--warn"),
                                       "warn if truncated or not found",
                                       ArgType_Flag, false, ArgValue(true));
  ...
  ds_write_cmd->add_positional_arg(
      "dsn", "data set name, optionally with member specified", ArgType_Single,
      true);
  ds_write_cmd->add_keyword_arg("encoding", encoding_option,
                                "encoding for input data", ArgType_Single,
                                false);
  ds_write_cmd->add_keyword_arg(
      "etag", etag_option,
      "Provide the e-tag for a write response to detect conflicts before save",
      ArgType_Single, false);
  ds_write_cmd->add_keyword_arg(
      "etag-only", etag_only_option,
  ...
  ds_delete_cmd->add_positional_arg(
      "dsn", "data set name, optionally with member specified", ArgType_Single,
      true);
  ...
  // Restore subcommand
  auto ds_restore_cmd =
      command_ptr(new Command("restore", "restore/recall data set"));
  ds_restore_cmd->add_positional_arg("dsn", "data set name", ArgType_Single,
                                     true);
  ds_restore_cmd->set_handler(handle_data_set_restore);
  data_set_cmd->add_command(ds_restore_cmd);

  // Compress subcommand
  auto ds_compress_cmd =
      command_ptr(new Command("compress", "compress data set"));
  ds_compress_cmd->add_positional_arg("dsn", "data set to compress",
                                      ArgType_Single, true);
  ...
  auto tool_convert_dsect_cmd =
      command_ptr(new Command("ccnedsct", "convert dsect to c struct"));
  tool_convert_dsect_cmd->add_keyword_arg(
      "adata-dsn", make_aliases("--adata-dsn", "--ad"), "input adata dsn",
      ArgType_Single, true);
  tool_convert_dsect_cmd->add_keyword_arg(
      "chdr-dsn", make_aliases("--chdr-dsn", "--cd"), "output chdr dsn",
      ArgType_Single, true);
  tool_convert_dsect_cmd->add_keyword_arg(
      "sysprint", make_aliases("--sysprint", "--sp"), "sysprint output",
      ArgType_Single, false);
  tool_convert_dsect_cmd->add_keyword_arg(
      "sysout", make_aliases("--sysout", "--so"), "sysout output",
      ArgType_Single, false);
  tool_convert_dsect_cmd->set_handler(handle_tool_convert_dsect);
  tool_cmd->add_command(tool_convert_dsect_cmd);

  // Dynalloc subcommand
  auto tool_dynalloc_cmd =
      command_ptr(new Command("bpxwdy2", "dynalloc command"));
  tool_dynalloc_cmd->add_positional_arg("parm", "dynalloc parm string",
                                        ArgType_Single, true);
  tool_dynalloc_cmd->set_handler(handle_tool_dynalloc);
  tool_cmd->add_command(tool_dynalloc_cmd);

  // Display symbol subcommand
  auto tool_display_symbol_cmd =
      command_ptr(new Command("display-symbol", "display system symbol"));
  tool_display_symbol_cmd->add_positional_arg("symbol", "symbol to display",
                                              ArgType_Single, true);
  tool_display_symbol_cmd->set_handler(handle_tool_display_symbol);
  tool_cmd->add_command(tool_display_symbol_cmd);

  // Search subcommand
  auto tool_search_cmd =
      command_ptr(new Command("search", "search members for string"));
  tool_search_cmd->add_positional_arg("dsn", "data set to search",
                                      ArgType_Single, true);
  tool_search_cmd->add_positional_arg("string", "string to search for",
                                      ArgType_Single, true);
  tool_search_cmd->add_keyword_arg(
      "max-entries", make_aliases("--max-entries", "--me"),
      "max number of results to return before warning generated",
      ArgType_Single, false);
  tool_search_cmd->add_keyword_arg("warn", make_aliases("--warn"),
                                   "warn if truncated or not found",
                                   ArgType_Flag, false, ArgValue(true));
  ...
  tool_amblist_cmd->add_positional_arg(
      "dsn", "data containing input load modules", ArgType_Single, true);
  tool_amblist_cmd->add_keyword_arg(
      "control-statements", make_aliases("--control-statements", "--cs"),
  ...
  tool_run_cmd->add_positional_arg("program", "name of program to run",
                                   ArgType_Single, true);
  tool_run_cmd->add_keyword_arg(
      "dynalloc-pre", make_aliases("--dynalloc-pre", "--dp"),
      "dynalloc pre run statements", ArgType_Single, false);
  tool_run_cmd->add_keyword_arg(
      "dynalloc-post", make_aliases("--dynalloc-post", "--dt"),
      "dynalloc post run statements", ArgType_Single, false);
  tool_run_cmd->add_keyword_arg("in-dd", make_aliases("--in-dd", "--idd"),
                                "input ddname", ArgType_Single, false);
  tool_run_cmd->add_keyword_arg("input", make_aliases("--input", "--in"),
                                "input", ArgType_Single, false);
  tool_run_cmd->add_keyword_arg("out-dd", make_aliases("--out-dd", "--odd"),
  ...
  auto uss_response_format_bytes_option =
      make_aliases("--response-format-bytes", "--rfb");

  // Create-file subcommand
  auto uss_create_file_cmd =
      command_ptr(new Command("create-file", "create a USS file"));
  uss_create_file_cmd->add_positional_arg("file-path", "file path",
                                          ArgType_Single, true);
  uss_create_file_cmd->add_keyword_arg("mode", make_aliases("--mode"),
                                       "permissions", ArgType_Single, false);
  uss_create_file_cmd->set_handler(handle_uss_create_file);
  uss_cmd->add_command(uss_create_file_cmd);

  // Create-dir subcommand
  auto uss_create_dir_cmd =
      command_ptr(new Command("create-dir", "create a USS directory"));
  uss_create_dir_cmd->add_positional_arg("file-path", "file path",
                                         ArgType_Single, true);
  uss_create_dir_cmd->add_keyword_arg("mode", make_aliases("--mode"),
                                      "permissions", ArgType_Single, false);
  ...
  // List subcommand
  auto uss_list_cmd =
      command_ptr(new Command("list", "list USS files and directories"));
  uss_list_cmd->add_positional_arg("file-path", "file path", ArgType_Single,
                                   true);
  uss_list_cmd->add_keyword_arg("all", make_aliases("--all", "-a"),
                                "list all files and directories", ArgType_Flag,
                                false, ArgValue(false));
  uss_list_cmd->add_keyword_arg("long", make_aliases("--long", "-l"),
                                "list long format", ArgType_Flag, false,
                                ArgValue(false));
  ...
  uss_view_cmd->add_positional_arg("file-path", "file path", ArgType_Single,
                                   true);
  uss_view_cmd->add_keyword_arg("encoding", uss_encoding_option,
                                "return contents in given encoding",
                                ArgType_Single, false);
  uss_view_cmd->add_keyword_arg("response-format-bytes",
                                uss_response_format_bytes_option,
                                "returns the response as raw bytes",
                                ArgType_Flag, false, ArgValue(false));
  uss_view_cmd->add_keyword_arg(
      "return-etag", uss_return_etag_option,
      "Display the e-tag for a read response in addition to data", ArgType_Flag,
      false, ArgValue(false));
  uss_view_cmd->add_keyword_arg(
      "pipe-path", uss_pipe_path_option,
      "Specify a FIFO pipe path for transferring binary data", ArgType_Single,
      false);
  ...
  uss_write_cmd->add_positional_arg("file-path", "file path", ArgType_Single,
                                    true);
  uss_write_cmd->add_keyword_arg("encoding", uss_encoding_option,
                                 "encoding for input data", ArgType_Single,
                                 false);
  uss_write_cmd->add_keyword_arg(
      "etag", uss_etag_option,
      "Provide the e-tag for a write response to detect conflicts before save",
      ArgType_Single, false);
  uss_write_cmd->add_keyword_arg(
      "etag-only", uss_etag_only_option,
      "Only print the e-tag for a write response (when successful)",
      ArgType_Flag, false, ArgValue(false));
  uss_write_cmd->add_keyword_arg(
      "pipe-path", uss_pipe_path_option,
  ...
  uss_delete_cmd->add_positional_arg("file-path", "file path", ArgType_Single,
                                     true);
  uss_delete_cmd->add_keyword_arg(
      "recursive", make_aliases("--recursive", "-r"),
      "Applies the operation recursively (e.g. for folders w/ inner files)",
      ArgType_Flag, false, ArgValue(false));
  uss_delete_cmd->set_handler(handle_uss_delete);
  uss_cmd->add_command(uss_delete_cmd);

  // Chmod subcommand
  auto uss_chmod_cmd = command_ptr(
      new Command("chmod", "change permissions on a USS file or directory"));
  uss_chmod_cmd->add_positional_arg("mode",
                                    "new permissions for the file or directory",
                                    ArgType_Single, true);
  uss_chmod_cmd->add_positional_arg("file-path", "file path", ArgType_Single,
                                    true);
  uss_chmod_cmd->add_keyword_arg(
      "recursive", make_aliases("--recursive", "-r"),
      "Applies the operation recursively (e.g. for folders w/ inner files)",
      ArgType_Flag, false, ArgValue(false));
  ...
  // Chown subcommand
  auto uss_chown_cmd = command_ptr(
      new Command("chown", "change owner on a USS file or directory"));
  uss_chown_cmd->add_positional_arg(
      "owner", "New owner (or owner:group) for the file or directory",
      ArgType_Single, true);
  uss_chown_cmd->add_positional_arg("file-path", "file path", ArgType_Single,
                                    true);
  uss_chown_cmd->add_keyword_arg(
      "recursive", make_aliases("--recursive", "-r"),
      "Applies the operation recursively (e.g. for folders w/ inner files)",
      ArgType_Flag, false, ArgValue(false));
  ...
  // Chtag subcommand
  auto uss_chtag_cmd =
      command_ptr(new Command("chtag", "change tags on a USS file"));
  uss_chtag_cmd->add_positional_arg("file-path", "file path", ArgType_Single,
                                    true);
  uss_chtag_cmd->add_positional_arg("tag", "new tag for the file",
                                    ArgType_Single, true);
  uss_chtag_cmd->add_keyword_arg(
      "recursive", make_aliases("--recursive", "-r"),
      "Applies the operation recursively (e.g. for folders w/ inner files)",
      ArgType_Flag, false, ArgValue(false));
  ...
  job_list_cmd->add_keyword_arg("owner", make_aliases("--owner", "-o"),
                                "filter by owner", ArgType_Single, false);
  job_list_cmd->add_keyword_arg("prefix", make_aliases("--prefix", "-p"),
                                "filter by prefix", ArgType_Single, false);
  job_list_cmd->add_keyword_arg(
      "max-entries", make_aliases("--max-entries", "--me"),
      "max number of results to return before warning generated",
      ArgType_Single, false);
  job_list_cmd->add_keyword_arg("warn", make_aliases("--warn"),
                                "warn if truncated or not found", ArgType_Flag,
                                false, ArgValue(true));
  job_list_cmd->add_keyword_arg("response-format-csv",
                                response_format_csv_option,
                                "returns the response in CSV format",
                                ArgType_Flag, false, ArgValue(false));
  ...
  auto job_list_files_cmd =
      command_ptr(new Command("list-files", "list spool files for jobid"));
  job_list_files_cmd->add_alias("lf");
  job_list_files_cmd->add_positional_arg(
      "jobid", "valid jobid or job correlator", ArgType_Single, true);
  job_list_files_cmd->add_keyword_arg(
      "max-entries", make_aliases("--max-entries", "--me"),
      "max number of files to return before warning generated", ArgType_Single,
      false);
  job_list_files_cmd->add_keyword_arg("warn", make_aliases("--warn"),
                                      "warn if truncated or not found",
                                      ArgType_Flag, false, ArgValue(true));
  job_list_files_cmd->add_keyword_arg("response-format-csv",
                                      response_format_csv_option,
                                      "returns the response in CSV format",
                                      ArgType_Flag, false, ArgValue(false));
  ...
  auto job_view_status_cmd =
      command_ptr(new Command("view-status", "view job status"));
  job_view_status_cmd->add_alias("vs");
  job_view_status_cmd->add_positional_arg(
      "jobid", "valid jobid or job correlator", ArgType_Single, true);
  job_view_status_cmd->add_keyword_arg("response-format-csv",
                                       response_format_csv_option,
                                       "returns the response in CSV format",
                                       ArgType_Flag, false, ArgValue(false));
  ...
  auto job_view_file_cmd =
      command_ptr(new Command("view-file", "view job file output"));
  job_view_file_cmd->add_alias("vf");
  job_view_file_cmd->add_positional_arg(
      "jobid", "valid jobid or job correlator", ArgType_Single, true);
  job_view_file_cmd->add_positional_arg(
      "key", "valid job dsn key via 'job list-files'", ArgType_Single, true);
  job_view_file_cmd->add_keyword_arg("encoding", encoding_option,
                                     "return contents in given encoding",
                                     ArgType_Single, false);
  job_view_file_cmd->add_keyword_arg("response-format-bytes",
                                     response_format_bytes_option,
                                     "returns the response as raw bytes",
                                     ArgType_Flag, false, ArgValue(false));
  ...
  // View-jcl subcommand
  auto job_view_jcl_cmd =
      command_ptr(new Command("view-jcl", "view job jcl from input jobid"));
  job_view_jcl_cmd->add_alias("vj");
  job_view_jcl_cmd->add_positional_arg("jobid", "valid jobid or job correlator",
                                       ArgType_Single, true);
  ...
  job_submit_cmd->add_positional_arg("dsn", "dsn containing JCL",
                                     ArgType_Single, true);
  job_submit_cmd->add_keyword_arg("wait", make_aliases("--wait"),
                                  "wait for job status", ArgType_Single, false);
  job_submit_cmd->add_keyword_arg(
      "only-jobid", make_aliases("--only-jobid", "--oj"),
      "show only job id on success", ArgType_Flag, false, ArgValue(false));
  job_submit_cmd->add_keyword_arg("only-correlator",
                                  make_aliases("--only-correlator", "--oc"),
                                  "show only job correlator on success",
                                  ArgType_Flag, false, ArgValue(false));
  job_submit_cmd->set_handler(handle_job_submit);
  job_cmd->add_command(job_submit_cmd);

  // Submit-jcl subcommand
  auto job_submit_jcl_cmd =
      command_ptr(new Command("submit-jcl", "submit JCL contents directly"));
  job_submit_jcl_cmd->add_alias("subj");
  job_submit_jcl_cmd->add_keyword_arg("wait", make_aliases("--wait"),
                                      "wait for job status", ArgType_Single,
                                      false);
  job_submit_jcl_cmd->add_keyword_arg(
      "only-jobid", make_aliases("--only-jobid", "--oj"),
      "show only job id on success", ArgType_Flag, false, ArgValue(false));
  job_submit_jcl_cmd->add_keyword_arg("only-correlator",
                                      make_aliases("--only-correlator", "--oc"),
                                      "show only job correlator on success",
                                      ArgType_Flag, false, ArgValue(false));
  job_submit_jcl_cmd->add_keyword_arg("encoding", encoding_option,
                                      "encoding for input data", ArgType_Single,
                                      false);
  ...
  auto job_submit_uss_cmd =
      command_ptr(new Command("submit-uss", "submit a job from USS files"));
  job_submit_uss_cmd->add_alias("sub-u");
  job_submit_uss_cmd->add_positional_arg("file-path", "USS file containing JCL",
                                         ArgType_Single, true);
  job_submit_uss_cmd->add_keyword_arg("wait", make_aliases("--wait"),
                                      "wait for job status", ArgType_Single,
                                      false);
  job_submit_uss_cmd->add_keyword_arg(
      "only-jobid", make_aliases("--only-jobid", "--oj"),
      "show only job id on success", ArgType_Flag, false, ArgValue(false));
  job_submit_uss_cmd->add_keyword_arg("only-correlator",
                                      make_aliases("--only-correlator", "--oc"),
                                      "show only job correlator on success",
                                      ArgType_Flag, false, ArgValue(false));
  ...
  job_delete_cmd->add_positional_arg("jobid", "valid jobid or job correlator",
                                     ArgType_Single, true);
  ...
  job_cancel_cmd->add_positional_arg("jobid", "valid jobid or job correlator",
                                     ArgType_Single, true);
  job_cancel_cmd->add_keyword_arg("dump", make_aliases("--dump", "-d"),
                                  "Dump the cancelled jobs if waiting for "
                                  "conversion, in conversion, or in execution",
                                  ArgType_Flag, false, ArgValue(false));
  job_cancel_cmd->add_keyword_arg("force", make_aliases("--force", "-f"),
                                  "Force cancel the jobs, even if marked",
                                  ArgType_Flag, false, ArgValue(false));
  job_cancel_cmd->add_keyword_arg("purge", make_aliases("--purge", "-p"),
                                  "Purge output of the cancelled jobs",
                                  ArgType_Flag, false, ArgValue(false));
  job_cancel_cmd->add_keyword_arg(
      "restart", make_aliases("--restart", "-r"),
      "Request that automatic restart management automatically restart the "
      "selected jobs after they are cancelled",
      ArgType_Flag, false, ArgValue(false));
  ...
  job_hold_cmd->add_positional_arg("jobid", "valid jobid or job correlator",
                                   ArgType_Single, true);
  ...
  job_release_cmd->add_positional_arg("jobid", "valid jobid or job correlator",
                                      ArgType_Single, true);
...
int handle_console_issue(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not activate console: '" << console_name << "' rc: '"
         << rc << "'" << endl;
  ...
  if (0 != rc) {
    cerr << "Error: could not write to console: '" << console_name << "' rc: '"
         << rc << "'" << endl;
    cerr << "  Details: " << zcn.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
  ...
  if (wait) {
    ...
    if (0 != rc) {
      cerr << "Error: could not get from console: '" << console_name
           << "' rc: '" << rc << "'" << endl;
      cerr << "  Details: " << zcn.diag.e_msg << endl;
      return RTNCD_FAILURE;
    }
  ...
  if (0 != rc) {
    cerr << "Error: could not deactivate console: '" << console_name
         << "' rc: '" << rc << "'" << endl;
    cerr << "  Details: " << zcn.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_data_set_create(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not create data set: '" << dsn << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details:\n" << response << endl;
    return RTNCD_FAILURE;
  }
  ...
  if (start != string::npos && end != string::npos && end > start) {
    ...
    if (0 != rc) {
      cout << "Error: could not write to data set: '" << dsn << "' rc: '" << rc
           << "'" << endl;
...
int handle_data_set_create_member(const ParseResult &result) {
  ...
  if (start != string::npos && end != string::npos && end > start) {
    ...
    if (RTNCD_WARNING < rc || entries.size() == 0) {
      cout << "Error: could not create data set member: '" << dataset_name
           << "' rc: '" << rc << "'" << endl;
      cout << "  Details:\n" << zds.diag.e_msg << endl;
      return RTNCD_FAILURE;
    }
...
int handle_data_set_view(const ParseResult &result) {
  ...
  if (result.has_kw_arg("encoding")) {
    zut_prepare_encoding(result.find_kw_arg_string("encoding"),
                         &zds.encoding_opts);
  }
  ...
  if (has_pipe_path && !pipe_path.empty()) {
    ...
    if (0 != rc) {
      cerr << "Error: could not read data set: '" << dsn << "' rc: '" << rc
           << "'" << endl;
    ...
    bool response_format_bytes =
        result.find_kw_arg_bool("response-format-bytes");
...
int handle_data_set_list(const ParseResult &result) {
  ...
  if (RTNCD_SUCCESS == rc || RTNCD_WARNING == rc) {
    ...
    for (vector<ZDSEntry>::iterator it = entries.begin(); it != entries.end();
         ++it) {
      if (emit_csv) {
        ...
        if (attributes) {
          cout << left << setw(44) << it->name << " " << it->volser << " "
               << setw(4) << it->dsorg << " " << setw(6) << it->recfm << endl;
        } else {
  ...
  if (RTNCD_SUCCESS != rc && RTNCD_WARNING != rc) {
    cerr << "Error: could not list data set: '" << dsn << "' rc: '" << rc << "'"
         << endl;
    cerr << "  Details: " << zds.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_data_set_list_members(const ParseResult &result) {
  ...
  if (RTNCD_SUCCESS == rc || RTNCD_WARNING == rc) {
    for (vector<ZDSMem>::iterator it = members.begin(); it != members.end();
         ++it) {
...
int handle_data_set_write(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not write to data set: '" << dsn << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details: " << zds.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_data_set_delete(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not delete data set: '" << dsn << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details: " << zds.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_data_set_compress(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not write to dd: '"
         << "sysin"
         << "' rc: '" << rc << "'" << endl;
    cerr << "  Details: " << zds.diag.e_msg << endl;
    return RTNCD_FAILURE;
  ...
  if (0 != rc) {
    cerr << "Error: could not read from dd: '"
         << "sysprint"
         << "' rc: '" << rc << "'" << endl;
    cerr << "  Details: " << zds.diag.e_msg << endl;
    cerr << output << endl;
...
int handle_tool_convert_dsect(const ParseResult &result) {
  ...
  cout << adata_dsn << " " << chdr_dsn << " " << sysprint << " " << sysout
       << endl;

  vector<string> dds;
  dds.push_back("alloc fi(sysprint) path('" + sysprint +
                "') pathopts(owronly,ocreat,otrunc) "
                "pathmode(sirusr,siwusr,sirgrp) filedata(text) msg(2)");
  ...
  if (0 != rc) {
    ...
    cout << "  See '" << sysprint << "' and '" << sysout << "' for more details"
         << endl;
    return RTNCD_FAILURE;
  }
...
int handle_tool_dynalloc(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: bpxwdyn with parm '" << parm << "' rc: '" << rc << "'"
         << endl;
...
int handle_tool_display_symbol(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: asasymbf with parm '" << symbol << "' rc: '" << rc << "'"
         << endl;
    return RTNCD_FAILURE;
  }
  cout << value << endl;
...
int handle_tool_search(const ParseResult &result) {
  ...
  // Perform search
  rc = zut_search("parms are unused for now but can be passed to super c, e.g. "
                  "ANYC (any case)");
  if (rc != RTNCD_SUCCESS || rc != ZUT_RTNCD_SEARCH_SUCCESS ||
      rc != RTNCD_WARNING || rc != ZUT_RTNCD_SEARCH_WARNING) {
...
int handle_tool_amblist(const ParseResult &result) {
  ...
  transform(statements.begin(), statements.end(), statements.begin(),
            ::toupper);
...
int handle_tool_run(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: program '" << program << "' ended with rc: '" << rc << "'"
         << endl;
    rc = RTNCD_FAILURE;
  }
...
int handle_uss_create_file(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not create USS file: '" << file_path << "' rc: '"
         << rc << "'" << endl;
...
int handle_uss_create_dir(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not create USS directory: '" << file_path << "' rc: '"
         << rc << "'" << endl;
    cerr << "  Details:\n" << zusf.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_uss_list(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not list USS files: '" << uss_file << "' rc: '" << rc
         << "'" << endl;
...
int handle_uss_view(const ParseResult &result) {
  ...
  if (has_pipe_path && !pipe_path.empty()) {
    ...
    if (0 != rc) {
      cerr << "Error: could not view USS file: '" << uss_file << "' rc: '" << rc
           << "'" << endl;
      cerr << "  Details:\n" << zusf.diag.e_msg << endl << response << endl;
      return RTNCD_FAILURE;
    }
    ...
    if (result.find_kw_arg_bool("return-etag")) {
      cout << "etag: "
           << zut_build_etag(file_stats.st_mtime, file_stats.st_size) << endl;
      cout << "data: ";
    }
...
int handle_uss_write(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not write to USS file: '" << file << "' rc: '" << rc
         << "'" << endl;
  ...
  if (result.find_kw_arg_bool("etag-only")) {
    ...
    cout << "Wrote data to '" << file << "'"
         << (zusf.created ? " (created new file)" : " (overwrote existing)")
         << endl;
  }
...
int handle_uss_chmod(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not chmod USS path: '" << file_path << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details:\n" << zusf.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_uss_chown(const ParseResult &result) {
  ...
  if (rc != 0) {
    cerr << "Error: could not chown USS path: '" << path << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details:\n" << zusf.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }

  cout << "USS path '" << path << "' owner changed to '" << owner << "'"
       << endl;
...
int handle_uss_chtag(const ParseResult &result) {
  ...
  if (rc != 0) {
    cerr << "Error: could not chtag USS path: '" << path << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details:\n" << zusf.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int job_submit_common(const ParseResult &result, string jcl, string &jobid,
                      string identifier) {
  int rc = 0;
  ZJB zjb = {0};
  ...
  if (0 != rc) {
    cerr << "Error: could not submit JCL: '" << identifier << "' rc: '" << rc
         << "'" << endl;
...
#define JOB_STATUS_INPUT "ACTIVE"
  ...
  if (JOB_STATUS_OUTPUT == wait || JOB_STATUS_INPUT == wait) {
    ...
    if (0 != rc) {
      cerr << "Error: could not wait for job status: '" << wait << "' rc: '"
           << rc << "'" << endl;
      cerr << "  Details: " << zjb.diag.e_msg << endl;
      return RTNCD_FAILURE;
    }
...
int handle_job_list(const ParseResult &result) {
  ...
  if (RTNCD_SUCCESS == rc || RTNCD_WARNING == rc) {
    ...
    for (vector<ZJob>::iterator it = jobs.begin(); it != jobs.end(); it++) {
      if (emit_csv) {
        ...
        cout << it->jobid << " " << left << setw(10) << it->retcode << " "
             << it->jobname << " " << it->status << endl;
      }
    }
  }
  if (RTNCD_WARNING == rc) {
  ...
  if (RTNCD_SUCCESS != rc && RTNCD_WARNING != rc) {
    cerr << "Error: could not list jobs for: '" << owner_name << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_job_list_files(const ParseResult &result) {
  ...
  if (RTNCD_SUCCESS == rc || RTNCD_WARNING == rc) {
    bool emit_csv = result.find_kw_arg_bool("response-format-csv");
    for (vector<ZJobDD>::iterator it = job_dds.begin(); it != job_dds.end();
         ++it) {
      ...
      if (emit_csv) {
        ...
        cout << left << setw(9) << it->ddn << " " << it->dsn << " " << setw(4)
             << it->key << " " << it->stepname << " " << it->procstep << endl;
      }
    }
  }
  ...
  if (RTNCD_SUCCESS != rc && RTNCD_WARNING != rc) {
    cerr << "Error: could not list files for: '" << jobid << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_job_view_status(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not view job status for: '" << jobid << "' rc: '"
         << rc << "'" << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
  ...
  if (emit_csv) {
    ...
    cout << job.jobid << " " << left << setw(10) << job.retcode << " "
         << job.jobname << " " << job.status << endl;
  }
  return 0;
}
...
int handle_job_view_file(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not view job file for: '" << jobid << "' with key '"
         << key << "' rc: '" << rc << "'" << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_job_view_jcl(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not view job file for: '" << jobid << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_job_submit_uss(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not view USS file: '" << file << "' rc: '" << rc
         << "'" << endl;
    cerr << "  Details:\n" << zusf.diag.e_msg << endl << response << endl;
    return RTNCD_FAILURE;
  }
...
int handle_job_delete(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not delete job: '" << jobid << "' rc: '" << rc << "'"
         << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_job_cancel(const ParseResult &result) {
  ...
  // Note: Cancel options (dump, force, purge, restart) are currently not used
  // by the backend but are defined for future compatibility
  ...
  if (0 != rc) {
    cerr << "Error: could not cancel job: '" << jobid << "' rc: '" << rc << "'"
         << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_job_hold(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not hold job: '" << jobid << "' rc: '" << rc << "'"
         << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int handle_job_release(const ParseResult &result) {
  ...
  if (0 != rc) {
    cerr << "Error: could not release job: '" << jobid << "' rc: '" << rc << "'"
         << endl;
    cerr << "  Details: " << zjb.diag.e_msg << endl;
    return RTNCD_FAILURE;
  }
...
int loop_dynalloc(vector<string> &list) {
  ...
  for (vector<string>::iterator it = list.begin(); it != list.end(); it++) {
    ...
    if (0 != rc) {
      cerr << "Error: bpxwdyn failed with '" << *it << "' rc: '" << rc << "'"
           << endl;
      cerr << "  Details: " << response << endl;
      return -1;
    }
...
bool should_quit(const std::string &input) {
  return (input == "quit" || input == "exit" || input == "QUIT" ||
          input == "EXIT");
}
