...
{
  string storageClass = "SC1"; // default to SC1 if the file does not exist
  // try {
  //   string properties_file = "test.properties.json";
  //   zjson::Value properties = read_json_file(properties_file);
  //   storageClass = properties["storageClass"].as_string();
  // } catch (const std::exception &e) {
  //   TestLog("Warning: Failed to read test.properties.json: " + string(e.what()));
  // }

  string response;
  string command = zowex_command + " data-set create " + ds_name + " " + ds_options + " --storclass " + storageClass;
