...
void zjb_tests() {
  ...
  describe("zjb tests", []() -> void {
    it("should be able to list a job", []() -> void {
      ...
      ExpectWithContext(rc, zjb.diag.e_msg)
          .ToBe(RTNCD_WARNING); // expect truncated list returned
    });
    ...
    it("should be able to view a submitted job", []() -> void {
      ...
      Expect(zjob.correlator)
          .ToBe(correlator); // vefify submit correlator matches view status
                             // correlator
    ...
    it("should be able to list and view SYSOUT files for INPUT jobs",
       []() -> void {
         ...
         ExpectWithContext(rc, zjb.diag.e_msg).ToBe(RTNCD_SUCCESS);
         Expect(dds.size())
             .ToBeGreaterThan(0); // expect at least one DD returned

         memset(&zjb, 0, sizeof(zjb));
         string content;
         rc =
             zjb_read_jobs_output_by_key(&zjb, correlator, dds[0].key, content);
         ExpectWithContext(rc, zjb.diag.e_msg).ToBe(RTNCD_SUCCESS);
