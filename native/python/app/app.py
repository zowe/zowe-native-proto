...
def list_data_sets():
    ...
    Query Parameters:
        dslevel: Data set name pattern (required)
        volser: Volume serial number (optional)
        start: Starting dataset name for pagination (optional)
    """
    try:
        ...
        return jsonify({"error": f"Error calling zds.list_data_sets: {str(e)}"}), 500
...
if __name__ == "__main__":
    print("Starting z/OS REST Files Service...")
    
    # Check if config file exists
    print(f"Looking for config file: {config_file_path}")
    print(f"Config file exists: {os.path.exists(config_file_path)}")
    
    # Try to register the service, but don't fail if Eureka is unavailable
    if enabler is not None:
        try:
            enabler.register()
            print("Successfully registered with Eureka discovery service")
        except Exception as e:
            print(f"Warning: Could not register with Eureka discovery service: {e}")
            print("Service will run without Eureka registration")
    else:
        print("Running without Eureka registration (enabler not initialized)")
