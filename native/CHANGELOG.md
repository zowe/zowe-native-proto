...
- **Breaking:** `c`: Removed the `zds_read_from_dd` function in favor of the newly-consolidated `zds_read` function. Callers should populate the `ZDS::ddname` variable before calling this function to prepare the read operation for a DD. [#831](https://github.com/zowe/zowe-native-proto/issues/831)
