    ...
    if (args[1]) {
        ...
        args.forEach((arg, index) => {
            if (0 === index) {
                ...
                let stats;
                try {
                    stats = statSync(resolve(__dirname, join(localDeployDir, arg)));
                } catch (e) {
                ...
                if (stats.isDirectory()) {
                    const files = readdirSync(resolve(__dirname, join(localDeployDir, arg)), {
                        withFileTypes: true,
                    });
                    files.forEach((entry) => {
                        if (!entry.isDirectory()) {
                            fileList.push(join(arg, entry.name));
                        }
                    });
                } else {
                    fileList.push(arg);
...
async function artifacts(connection: Client) {
    ...
    await runCommandInShell(
        ...
        [
            `cd ${deployDirectory}`,
            ...prePaxCmds,
            `pax -wzvf ${paxFile} ${artifactNames.map((file) => basename(file)).join(" ")}`,
            ...postPaxCmds,
        ].join("\n"),
    );
...
async function deploy(connection: Client) {
    return new Promise<void>((finish) => {
        ...
        connection.sftp(async (err, sftpcon) => {
            ...
            for (let i = 0; i < files.length; i++) {
                const from = resolve(__dirname, join(localDeployDir, files[i]));
...
async function build(connection: Client) {
    ...
    console.log(await runCommandInShell(connection, `cd ${cDeployDirectory} && make\n`));
