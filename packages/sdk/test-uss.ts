...
(async () => {
    ...
    console.log(JSON.stringify(sshProfAttrs));
    ...
    // ts-ignore
    const client = await ZSshClient.create(session, { serverPath: serverPathArg?.argValue });
    ...
    const response = await client.uss.copyUss({ srcFsPath: "~/test.sh", dstFsPath: "~/test.sh2" });
    console.log("done");
})().catch((err) => {
    console.error(err);
    process.exit(1);
