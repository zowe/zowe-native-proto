...
export class ZSshClient implements Disposable {
	private static readonly SERVER_CMD =
		"cd zowe-native-proto/golang && ./ioserver";
	...
	public async request<T extends IRpcResponse>(
		request: IRpcRequest,
		stream?: Writable,
	): Promise<T> {
		...
		return new Promise((resolve, reject) => {
			...
			this.mSshStream!.stdout.on(
				"data",
				this.onOutData.bind(this, (response: any) =>
	...
	private static buildConnectConfig(session: SshSession): ConnectConfig {
		return {
			...
			privateKey: session.ISshSession.privateKey
				? fs.readFileSync(session.ISshSession.privateKey)
				: undefined,
