...
- Added expired password detection (`FOTS1668`/`FOTS1669`) in `installServer` and `uninstallServer`, throwing an `ImperativeError` with `errorCode: "EPASSWD_EXPIRED"` instead of returning a generic failure.
