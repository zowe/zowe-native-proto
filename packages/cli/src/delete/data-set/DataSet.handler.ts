...
export default class DeleteDatasetHandler extends SshBaseHandler {
    public async processWithClient(params: IHandlerParameters, client: ZSshClient): Promise<ds.DeleteDatasetResponse> {
        const dsname = params.arguments.name;

        const response = await client.ds.deleteDataset({ dsname });

        const dsMessage = `Dataset "${dsname}" deleted`;
