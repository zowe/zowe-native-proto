...
- **The Exit Routine (`ZAMDEXIT`)**:
   ...
   - It evaluates the parameter list's options mask (`plist->option_mask`). If `DCB_ABEND_OPT_OK_TO_IGNORE` is set, it returns `DCB_ABEND_RC_IGNORE` to tell the system that it is safe to ignore the abend (e.g., an out-of-space SE37 abend). Otherwise, it defaults to returning `DCB_ABEND_RC_TERMINATE`. Some abends cannot be ignored but they can be delayed; if `DCB_ABEND_OPT_OK_TO_DELAY` is set, it returns `DCB_ABEND_RC_DELAY` and delays the abend until all the DCBs in the same OPEN or CLOSE macro are opened or closed.
...
- **The Context Pointer (`gioc`)**:
   ...
   - Prior to beginning any I/O operation (e.g., `OPEN` or `CLOSE`), `gioc` is pointed to the active `IO_CTRL` structure.
...
1. The underlying macro (e.g., `write_sync`, `stow`, or `close_dcb`) encounters a condition that triggers `ZAMDEXIT`.
...
This design enables consumers to handle error scenarios during BPAM operations without crashing the application and performing improper data set processing. **Note:** If an abend occurs during end-of-volume processing, the ignore request may be discarded and the abend could still propagate up to ESTAEX recovery.
